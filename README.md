# Deji

Data engineer and Python developer. I build data pipelines, Django/Flask web apps, and web scraping tools.

## What I work on

- **Data engineering**: ETL pipelines, Redshift/Postgres, async ingestion, Polars
- **Web development**: Django, Flask, REST APIs
- **Automation**: web scraping, third-party API integrations, workflow tooling

## Open source

My data engineering tools live under **[pipewell](https://github.com/pipewell)**.

| Project | Description |
|---|---|
| [jira-ingest](https://github.com/pipewell/jira-ingest) | Async Jira Cloud pipeline: fetches projects, boards, issues and transitions into any SQLAlchemy-compatible database |
| [confluence-publisher](https://github.com/pipewell/confluence-publisher) | GitHub Action that publishes Markdown docs to Confluence Cloud or Data Center |

## Languages and tools

Python · SQL · Django · Flask · Polars · SQLAlchemy · PostgreSQL · Redshift · Docker · GitHub Actions
